  ...
  stream.streamBlockOperations(ops => {
    ...
    if (op) {
      ...
      if (
        ...
        // we want to upvote any posts upvoted by @steemauto.app (include self posts)
        if (op[1].voter !== op[1].author || op[1].voter === 'steem-supporter') {
...
const trailupvote = async (userr, author, permlink, fweight) => {
  console.log("//start//////////////////////////////////////////")
  console.log('Owner:', userr, author, permlink, fweight);
  i++
  if (i < 1) i = 1
  try {
    setTimeout(async () => {
      ...
      if (nowtime - createdtime < 561600 && content.parent_author === '') {
        ...
        console.log('trail followers (shuffled):', results);
        for (let i in results) {
          ...
          if (voted === 0) {
            ...
            if (aftermin > 0) {
              ...
              console.log("result check limit", result)

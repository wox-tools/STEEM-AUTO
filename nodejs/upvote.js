...
const upvote = async (wif, voter, author, permlink, weight) => {
  console.log('actual voting')
  console.log('actual wif=', wif)
  console.log('actual voter=', voter)
  console.log('actual author=', author)
  console.log('actual permalink=', permlink)
    ...
    if (wif && voter && author && permlink && weight) {
      ...
      steem.api.getContentAsync(author, permlink)
        .then((result) => {
          ...
          if ((secondss - secondss1) < 561600) {
            ...
            if (voted === 0) {
              upvote(wif, voter, author, permlink, weight)
                .then(result => {
                  console.log('up vote result:', result);
                  k--
                ...
                .catch(err => {
                  console.log('up vote result Failed!!!');
                  k--
                  res.end(JSON.stringify({
                    result: 0,
                    reason: 'up fail',
              ...
              console.log('vote result: Already voted');
              k--
              res.end(JSON.stringify({
                result: 0,
            ...
            console.log('vote result: too old');
            k--
            res.end(JSON.stringify({
              result: 0,
              reason: 'too old!'
          ...
          console.log('vote result: rpc node fail', err);
          k--
          res.end(JSON.stringify({
            result: 0,

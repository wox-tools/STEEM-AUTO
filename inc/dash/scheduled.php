			...
			<div class="card">
				<div class="content">
					...
					<form style="display:;" id="post" onsubmit="post(); return false;">
						...
						<sub><a href="https://simplemde.com/markdown-guide" target="_blank">Markdown</a> supported</sub><br>
						<!-- tags -->
						...
						<label for="beneficiarytype">Optional beneficiary <abbr data-toggle="tooltip" title="You can donate certain percent of your posts reward to the steemauto. Check FAQ">?</abbr> 
							<select id="beneficiarytype" name="beneficiarytype">
								...
								<option value="1" selected>1% @wox-helpfund</option>
								<option value="5">5% @wox-helpfund</option>
								<option value="10">10% @wox-helpfund</option>
								<option value="15">15% @wox-helpfund</option>
								<option value="20">20% @wox-helpfund</option>
								<option value="25">25% @wox-helpfund</option>
							</select>
						</label>
						<br>
						...
						?>
						<hr>
						<label>Current date & time is <span style="color:green;"><?php echo $datenow; ?> (UTC)</span></label><br>
						  ...
						  <tbody>
								...
								?>
										<tr class="tr2">
											...
											<td data-title="Name"><?php echo $x['title']; ?></td>
											<td data-title="Status"><textarea disabled="" height="50px"><?php echo $x['content']; ?></textarea></td>
											<td data-title="Status"><?php echo after($x['date']); ?></td>
											<td data-title="Status"><?php if($x['status'] == 0){echo 'Waiting';}elseif($x['status'] == 1){echo 'Published';}else{echo 'Error<sup>*</sup>';} ?></td>
											<td data-title="Status"><button class="btn btn-danger" onclick="if(confirm('Are You Sure?')) deletepost('<?php echo $x['id']; ?>');">DELETE</button></td>
										</tr>

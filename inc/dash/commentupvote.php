							...
							<table id="table" class="table table-hover table-mc-light-blue">
							  ...
							  <tbody>
									...
									<tr class="tr1">
									  ...
									  <td data-title="Name"><a href="https://steemit.com/@<?php echo $n['commenter']; ?>" target="_blank">@<?php echo $n['commenter']; ?></a></td>
									  ...
									  <button onclick="showset('<?php echo $k; ?>');" class="btn btn-primary">Settings</button>
									  <button onclick="if(confirm('Are you sure?')){removeuserfromlist('<?php echo $n['commenter']; ?>');};" class="btn btn-danger">REMOVE</button>
									  </td>
									</tr>
									<!-- Settings -->
									<div class="row" style="margin:0 !important;">
										<div class="col-md-3"></div>
										<div style="text-align:left; display:none; padding:20px;" id="set<?php echo $k; ?>" class="col-md-6">
											<form onsubmit="commentupvotesettings('<?php echo $n['commenter']; ?>');return false;">
												<label>Settings for Commenter: <a href="https://steemit.com/@<?php echo $n['commenter']; ?>" target="_blank">@<?php echo $n['commenter']; ?></a></label>
												<br><label>Weight:</label>
												<input id="weight<?php echo $n['commenter']; ?>" placeholder="Voting Weight" name="weight" type="number" class="form-control" value="<?php echo $n['weight']/100; ?>" step="0.01" min="0.01" max="100" required/>
												<label>Time to wait before voting:</label>
												<input id="aftermin<?php echo $n['commenter']; ?>" value="<?php echo $n['aftermin']; ?>" placeholder="Upvoting After X Minutes." name="aftermin" type="number" class="form-control" step="1" min="0" max="30" required/>
												<li style="margin-top:5px; margin-bottom:5px;" class="list-group-item">
													Enabled:
													<div class="material-switch pull-right">
														<input id="enable<?php echo $n['commenter']; ?>" name="enable" class="enable" type="checkbox" <?php if($fc == 1){echo 'checked';} ?>/>
														<label for="enable<?php echo $n['commenter']; ?>" id="enable" class="label-success"></label>
													</div>
												</li>
